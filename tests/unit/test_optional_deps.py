...
def test_sqlalchemy_missing_error():
    ...
    with patch.dict("sys.modules", {"sqlalchemy": None, "sqlalchemy.orm": None, "sqlalchemy.sql": None}):
        ...
        with pytest.raises(ImportError) as excinfo:
            sa_adapter.apply_filters(None, None, None)
        assert "The 'sqlalchemy' extra is required" in str(excinfo.value)
